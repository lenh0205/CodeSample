...
namespace DotNetNuke.Entities.Host
{
    ...
    {
        /// <summary>Get the server's endpoint which can access the server directly.</summary>
        /// <returns>The URL or <see cref="string.Empty"/>.</returns>
        string GetServerUrl();

        /// <summary>Get the server's unique id when server is behind affinity tool.</summary>
        /// <returns>The ID.</returns>
        string GetServerUniqueId();

        /// <summary>Process Request before the request send to server.</summary>
