...
namespace Dnn.PersonaBar.Pages.Components
{
    ...
    public interface IPagesController
    {
        ...
        IEnumerable<TabInfo> GetPageList(PortalSettings portalSettings, bool? deleted, string tabName, string tabTitle, string tabPath, string tabSkin, bool? visible, int parentId, out int total, string searchKey = "", int pageIndex = -1, int pageSize = 10, bool includeSubpages = false);

        IEnumerable<TabInfo> SearchPages(out int totalRecords, string searchKey = "", string pageType = "", string tags = "", string publishStatus = "", string publishDateStart = "", string publishDateEnd = "", int workflowId = -1, int pageIndex = -1, int pageSize = -1);
        ...
        /// <summary>Returns a clean tab relative url based on Advanced Management Url settings.</summary>
        ...
        /// <summary>Copy the given theme to all descendant pages.</summary>
        /// <param name="pageId">page identifier.</param>
        /// <param name="theme">Theme.</param>
        void CopyThemeToDescendantPages(int pageId, Theme theme);

        /// <summary>Copy the current page permissions to all descendant pages.</summary>
        /// <param name="pageId">page identifier.</param>
